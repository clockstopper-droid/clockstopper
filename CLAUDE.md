...
The app updates all displayed clocks every second and allows users to dynamically add or remove time zones via a simple UI. A **dark theme with orange accent keypad/controls** is supported via a CSS class toggle.
...
├── js/
│   └── app.js          # All application logic, theme toggle
...
js/app.js
  └── State management    → Tracks list of active time zones
...
- Defines the page structure: a heading, the `#clocksGrid` div (dynamically populated), the controls section, and a **theme toggle button**.
- Loads `css/style.css` and `js/app.js` via relative paths.
- Uses inline `onclick="addTimezone()"` on the Add Clock button and `onclick="toggleTheme()"` on the theme toggle button — both functions must be globally scoped in `app.js`.
...
### `js/app.js`
- Contains **all application logic** including theme toggling.
- Manages the time zone state array, DOM rendering, the update interval, add/remove operations, and the dark theme toggle.
- Uses the `Intl.DateTimeFormat` API for locale-aware, time-zone-aware formatting — no external date libraries needed.
- `toggleTheme()` must remain globally scoped as it is referenced via an HTML `onclick` attribute.

### `Css/Style.css`
- Handles all visual presentation including the **dark theme**.
...
- **Global functions** — `addTimezone()`, `removeTimezone()`, and `toggleTheme()` must remain globally accessible because they are referenced in HTML `onclick` attributes.
...
- **Theme toggling via CSS class** — use `document.body.classList.toggle()` pattern; avoid inline styles for theming.
...
| Global function scope | `addTimezone()`, `removeTimezone()`, and `toggleTheme()` must not be wrapped in a module or block scope |
